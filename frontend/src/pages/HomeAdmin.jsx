  ...
  return (
    <div className="bg-slate-50 text-slate-800">
      ...
      <aside className="fixed inset-y-0 left-0 w-64 bg-white/95 backdrop-blur border-r border-slate-200 shadow-sm z-40">
        ...
        {/* menú (puede scrollear si crece) */}
        <nav className="py-3 px-2 overflow-y-auto bg-teal-400 h-[calc(100vh-4rem)]">
          {[
            "Productos","Ventas","Cocina","Utensilios","Materias primas","Bodegas",
            "Gestión sedes","Usuarios","Producción","Control de gastos","Proveedores",
            "Clientes","Áreas de venta","Estadísticas","Software utilidades",
          ].map((label, idx) => (
            <button
              key={label}
              className={`w-full text-left flex items-center gap-3 px-3 py-2 rounded-lg mb-1
                          ${idx===1 ? "bg-sky-50 text-sky-700 border border-sky-200" : "hover:bg-slate-100 text-slate-700"}`}
              type="button"
            >
      ...
      <main className="ml-64 h-screen overflow-y-auto">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="h-16 flex items-center justify-between">
              <div className="text-sm">
                ...
                <span className="align-middle bg-teal-600/90 text-white px-2 py-0.5 rounded text-[11px] font-medium">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 space-y-8">
          ...
          {/* ===== Header + filtros + Acumulado del mes ===== */}
          ...
          {/* ===== Sección media: categorías + % y usuarios ===== */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <Pie
                      data={[120642, 413876]}
                      labels={[" JULIANA HOYOS ", "karen hoyos"]}
...
/* =========================================================================================
   Helpers / mini-componentes (todo en este archivo para tu preferencia de archivo único)
========================================================================================= */
...
function StatLine({ label, value, emphasized = false }) {
  return (
    ...
    >
      <span className={`text-sm ${emphasized ? "text-teal-700 font-semibold" : "text-slate-600"}`}>
        {label}
      </span>
      <span className={`font-semibold ${emphasized ? "text-teal-800" : "text-slate-800"}`}>
...
/* ===== Top productos con gráfico + tabla ===== */
          ...
          <div className="pt-4 flex items-center justify-between text-xs text-slate-500">
            ...
            <div className="flex items-center gap-1">
              {["<<", "<", "1", "2", "3", "4", ">", ">>"].map((b, idx) => (
                <button
                  key={idx}
                  className={`px-2 py-1 rounded border ${b === "1" ? "bg-slate-100 font-medium" : ""}`}
                  type="button"
                >
                  {b}
                </button>
